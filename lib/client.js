...
`;
		...
		function GatewaySection({ t }) {
			...
			const openCreateGroup = react.useCallback(() => {
				...
				setGroupDialog({ mode: "create", name: "", models: [] });
			}, []);

			/** Open the dialog seeded from an existing group. */
			const openEditGroup = react.useCallback((group) => {
				setGroupError(undefined);
				setGroupDialog({ mode: "edit", id: group.name, name: group.name, models: [...group.models] });
			}, []);
			...
			const submitGroup = react.useCallback(async () => {
				...
				const result = groupDialog.mode === "edit"
					? await callSettings("updateGroup", { id: groupDialog.id, name: groupDialog.name, models: groupDialog.models })
					: await callSettings("createGroup", { name: groupDialog.name, models: groupDialog.models });
				setGroupBusy(false);
				if (!result.ok) {
