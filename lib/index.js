  ...
  const openStream = async (route, baseOptions, signal) => {
    ...
    for (const attempt of attempts) {
      ...
      for (let retry = 0; ; retry += 1) {
        ...
        if (route.group !== undefined) stats.attempt(route.exposed, bound.exposed)
        ...
        try {
          for (;;) {
            ...
            if (next.done === true) {
              if (route.group !== undefined) stats.answered(route.exposed, bound.exposed)
            ...
            if (outcome !== undefined) {
              if (outcome === 'aborted' || committed) {
                ...
                if (route.group !== undefined) {
                  ...
                  if (outcome === 'aborted') stats.excused(route.exposed, bound.exposed)
                  else stats.answered(route.exposed, bound.exposed)
                }
                return { stream: resume(buffered, iterator), served: bound }
              }
              failure = outcome
            ...
            if (committed || route.group === undefined) {
              if (committed && route.group !== undefined) stats.answered(route.exposed, bound.exposed)
              return { stream: resume(buffered, iterator), served: bound }
            }
          }
        } catch (error) {
        ...
        if (route.group !== undefined && !committed) {
          stats.refused(route.exposed, bound.exposed, failure?.code)
        }
        ...
        if (retry < retry429 && !committed && isRetryableRateLimit(failure)) {
          ...
          if (wait !== undefined) {
            if (route.group !== undefined) stats.rateLimited(route.exposed, bound.exposed)
  ...
  async function* streamGroup(groupName, options) {
    ...
    for (const leg of legs) {
      ...
      for (let retry = 0; ; retry += 1) {
        ...
        stats.attempt(groupName, bound.exposed)
        ...
        try {
          for (;;) {
            const next = await iterator.next()
            if (next.done === true) {
              stats.answered(groupName, bound.exposed)
              return
            }
            const outcome = terminalOutcome(next.value)
            if (outcome !== undefined) {
              ...
              if (outcome === 'aborted' || started) {
                ...
                // counted as answered, so both paths report one definition:
                // "did this candidate answer". See `lib/stats.js`.
                if (outcome === 'aborted') stats.excused(groupName, bound.exposed)
                else stats.answered(groupName, bound.exposed)
                yield next.value
                return
              }
              sawFailure = outcome
          ...
          if (signal?.aborted === true) {
            stats.excused(groupName, bound.exposed)
            throw error
          }
          lastError = error
          failure = error
          ...
          if (started) {
            // Counted as answered, not refused: the candidate did answer, and
            // `openStream` cannot tell this case apart from a clean finish at
            // all. One definition on both paths is worth the lost detail.
            stats.answered(groupName, bound.exposed)
            throw error
          }
        } finally {
        ...
        if (!started) stats.refused(groupName, bound.exposed, failure?.code)
        ...
        if (retry < retry429 && !started && isRetryableRateLimit(failure)) {
          const wait = retryDelayMs(retry + 1, failure?.providerRetryAfterMs, spentMs)
          if (wait !== undefined) {
            stats.rateLimited(groupName, bound.exposed)
