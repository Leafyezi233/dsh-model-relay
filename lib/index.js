...
/**
 * Translate one OpenAI message into one or more DSH messages.
 *
 * OpenAI's `tool` role becomes a DSH tool-result message on the user role; an
 * assistant turn carrying `tool_calls` becomes tool-call blocks. System
 * messages stay system messages so the provider adapter can place them itself.
 *
 * @param message - the OpenAI message.
 * @param attachments - optional attachment service for image parts.
 * @returns DSH messages in order.
 */
async function toDshMessages(message, attachments, signal) {
  ...
  if (role === 'system' || role === 'developer') {
    ...
    return [createSystemMessage(text, name)]
  }
  if (role === 'tool') {
...
export function apply(ctx, config = {}) {
  ...
  const buildOptions = async (body, signal) => {
    ...
    for (const message of rawMessages) {
      messages.push(...await toDshMessages(message, attachments, signal))
    }
