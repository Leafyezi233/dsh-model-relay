...
export function apply(ctx, config = {}) {
  ...
  const resolveRoute = async (requested, signal) => {
    ...
    if (group !== undefined && group.enabled) {
      const legs = group.models.filter((model) => model !== raw)
      if (legs.length === 0) {
        throw new GatewayError(500, `group "${raw}" has no usable candidates`, 'api_error', 'group_empty')
      }
      // Resolving a leg uses the same path as any explicit model, so a group
      // member is spelled exactly like what /v1/models advertises.
      const head = await resolveLeg(legs[0], signal)
      return { ...head, exposed: raw, group: { id: group.id, name: group.name, legs } }
    }
  ...
  const dispatch = async (action, payload) => {
    switch (action) {
      ...
      case 'createGroup': {
        ...
        ctx.logger.info(`dsh-model-relay: created model group ${JSON.stringify(result.group.name)} (${result.group.models.length} candidates)`)
